...
?>
	...
	<body>
		...
		$getInfoped = $database->getPedidobyID($idpe);
		$getInfoCli = $database->getClientesbyId($getInfoped[0]['idcliente']);
		?>
		<div class="container">
			<div class="row ">
	        ...
	        <form name="pedido" id="pedido" action="../../process.php" method="post" >
        		<input type="hidden" size="1" readonly name="action" id="action" value="modifPed"/>
        		<input type="hidden" size="1" readonly name="idpe" id="idpe" value="<?php echo $idpe; ?>"/>
				<?php
					$validcli = $session->checkValueVar($form->value('idcliente'),$getInfoped[0]['idcliente']);
					...
					$valfecha= $session->checkValueVar($form->value('fecha'),$getInfoped[0]['fecha']);
					$valNorden = $session->checkValueVar($form->value('Norden'),$getInfoped[0]['Norden']);
				?>
